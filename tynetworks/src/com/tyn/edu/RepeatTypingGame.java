...
class RepeatTypingGame extends Frame {
	
	Panel pScore = new Panel(new GridLayout(1, 3));
	
	public static void main(String[] args) {
		RepeatTypingGame win = new RepeatTypingGame();
		win.start();
	}
	
	RepeatTypingGame() {
		this("Typing game ver1.0"); // 인자 하나짜리 생성자 호출
	} // 기본 생성자

	RepeatTypingGame(String title) {
		super(title); // 슈퍼클래스의 인자 하나짜리 생성자와 연결하기 위해. 안하면 슈퍼클래스의 인자 없는 생성자로 연결됨.
