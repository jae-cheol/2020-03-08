...
public class 말이되고픈원숭이 {
	...
	public static void bfs() {
		...
		while(q.peek() != null) {
			...
			if(ability < k) {
				for(int i=0; i<8; i++) {
					int nx = xPoint + horse[i][0];
					int ny = yPoint + horse[i][1];

					if(nx>=0 && ny>=0 && nx<h && ny<w) {
						if(area[nx][ny] == 0 && !checked[nx][ny][ability+1]) {
							checked[nx][ny][ability+1] = true;
							q.offer(new Point(nx, ny, cnt+1, ability+1));
						}
					}
				}
			}
			...
			// 원숭이처럼 이동할 경우.
			for(int i=0; i<4; i++) {
				int nx = xPoint + monkey[i][0];
				int ny = yPoint + monkey[i][1];
				
				if(nx>=0 && ny>=0 && nx<h && ny<w) {
					if(area[nx][ny] == 0 && !checked[nx][ny][ability]) {
						checked[nx][ny][ability] = true;
						q.offer(new Point(nx, ny, cnt+1, ability));
					}
				}
			}
		}
